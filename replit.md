...
- Sorted file organization update: internal sorted files are grouped into specialty/keyword sections such as surgery, internal medicine, pediatrics, fellowship/residency, and other configured specialty keys. Missing member counts are saved as `غير متاح` and do not stop sorting.
...
- **Bot stats and cleanup commands**: `/stats` and the dashboard stats button now show raw, unique raw, per-category sorted, archived, broken, cached, joined, malformed, and retry-pending counts. `/fixlinks` and the stats screen cleanup button retroactively rewrite malformed stored Telegram links to canonical `https://t.me/...` format across raw links, seen/archived files, inspection cache, sorted files, and bot memory.
